...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [users, customers, transactions, events] = await Promise.all([
          adminService.getAllUsers(),
          adminService.getAllCustomers(),
          adminService.getAllTransactions(0, 1),
          adminService.getAuditEvents({ page: 0, size: 1 }),
        ]);
        setUserCount(users?.length || 0);
        setCustomerCount(customers?.length || 0);
        setTransactionCount(transactions?.totalElements || 0);
        setEventCount(events?.totalElements || 0);
      } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <StatsCard
          ...
          color="cyan"
        >
          <p className="text-3xl font-bold text-slate-50">{eventCount}</p>
        </StatsCard>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <QuickLink
          ...
          description="Browse all system transactions"
        />
        <QuickLink
          href="/dashboard/users"
          title="Manage Users"
          description="Create and manage staff users"
        />
        <QuickLink
        ...
        <QuickLink
          ...
          title="View Events"
          description="Real-time audit event stream"
        />
      </div>
    </div>
  );
...
function QuickLink({
  ...
  return (
    <Link
      href={href}
      className="glass-card p-6 rounded-xl hover:border-primary-500/50 hover:bg-primary-500/5 transition-all group"
    >
